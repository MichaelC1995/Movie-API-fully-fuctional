...
const menuElement = document.querySelector(`modal`);
...
async function displayMovieList(movies, filter = "") {
  ...
  for (let i = 0; i < 6; ++i) {
    ...
    movieListItem.dataset.id = filteredMovies[i].imdbID;
    movieListItem.classList.add("search__list__item");
    if (filteredMovies[i].Poster !== "N/A") {
      moviePoster = filteredMovies[i].Poster;
    } else {
  ...
  searchListMovies.forEach((movie) => {
    movie.addEventListener("click", async () => {
      ...
      const result = await fetch(
        `https://www.omdbapi.com/?i=${movie.dataset.id}&apikey=cc5c9783`
      );
...
async function filterMovies(event) {
  ...
  const response = await fetch(`${URL}`);
  const data = await response.json();
  displayMovieList(data.Search, event.target.value);
}
